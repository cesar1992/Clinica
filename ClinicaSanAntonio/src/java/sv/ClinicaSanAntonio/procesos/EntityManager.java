...
public class EntityManager {
    ...
    private Field[] getCampos() throws ClassNotFoundException, NoSuchMethodException{
      Class<?> c;
    ...
    private String getListaCampos(){
        ...
        for (int i =0;i<f.length;i++){
            if (AUTO_INCREMENT && f[i].getName().equals(PRIMARY_KEY.toString())){
    ...
    private String getListaValores(){
        ...
        int i =0;
        for (String v: aux){
            nombres_metodos[i] = "get" + String.valueOf(v.charAt(0)).toUpperCase() + v.substring(1);
            i++;
        }
        try{
    ...
    protected String getSelect(String id){
        String sqlinsert = "select " + getTodosCampos()+ " from " + NombreEntidad + " where "+PRIMARY_KEY.toString()+" = '" + id+ "'";
        return sqlinsert;
    }
    
    protected String getInsert() {
        String sqlinsert = "insert into " + NombreEntidad + " (" + getListaCampos() + ")" + " values ("+ getListaValores() + ")";
        return sqlinsert;
    }
    
    protected String getUpdate(String id){
        String sqlupdate = "update " + NombreEntidad + " set ";
    ...
    protected String getDelete(String id) {
        String sqldelete = "delete from " + NombreEntidad + " where "+PRIMARY_KEY.toString()+" = '" + id+ "'";
        return sqldelete;
    }
    
    protected String getDelete(String[] id) {
       String sqldelete = "delete from " + NombreEntidad + " where "+PRIMARY_KEY.toString()+" in ("+ EntityManager.join(id) +")";
        return sqldelete; 
    }
